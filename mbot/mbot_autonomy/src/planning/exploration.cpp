...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    // Select which frontier to explore if there is any left
    if (!initialized_ && frontiers_.size() == 0)
        return mbot_lcm_msgs::exploration_status_t::STATE_EXPLORING_MAP;
    else
        initialized_ = true;
    
    // Decide which frontier to follow and get the path to it
    auto front_processing = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    // check if the home position is a valid goal, and search for the closest valid if it isn't.
    int max_counter_dist = 4;
    // Number of directions to search from the home cell
    ...
    auto home_cell = global_position_to_grid_cell(Point<double>(homePose_.x, homePose_.y), currentMap_);
    auto path_home = planner_.planPath(currentPose_, homePose_);
    if (path_home.path_length > 1)
        currentPath_ = path_home;
    else {
        printf("Home position not available\n");
    }
    // else
    // {
    //     cell_t goal_cell = home_cell;
    //     bool found_goal = false;
    //     for (size_t i = 1; i < max_counter_dist; i++)
    //     {
    //         for (auto &&dir : directions)
    //         {
    //             cell_t new_cell(
    //                 home_cell.x + dir.first * i,
    //                 home_cell.y + dir.second * i
    //             );
    //             // Check if it is a valid goal

    //             auto path_found = planner_.planPath()
    //             if (planner_.isValidGoal(new_cell))
    //             {
    //                 goal_cell = new_cell;
    //                 found_goal = true;
    //                 printf("Found valid goal!\n");
    //                 break;
    //             }
    //         }
    //         if (found_goal) break;
    //         printf("last home: (%f,%f). New home: (%f,%f)\n", homePose_.x, homePose_.y, new_home_pose.x, new_home_pose.y);
    //     }

        
    //     currentPath_ = planner_.planPath(currentPose_, new_home_pose);
    // }
